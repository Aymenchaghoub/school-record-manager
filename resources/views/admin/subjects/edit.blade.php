...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600">Update subject information for <span class="font-semibold">{{ $subject->name }}</span></p>
        </div>
        <x-modern.button
        ...
        <form action="{{ route('admin.subjects.update', $subject->id) }}" method="POST" class="space-y-6">
            ...
            <div class="bg-gray-50 rounded-xl p-4 space-y-2">
                <div class="flex items-center justify-between text-sm">
                    ...
                    <span class="font-medium text-gray-900">{{ $subject->created_at->format('M d, Y h:i A') }}</span>
                </div>
                <div class="flex items-center justify-between text-sm">
                    <span class="text-gray-600">Last Updated:</span>
                    <span class="font-medium text-gray-900">{{ $subject->updated_at->format('M d, Y h:i A') }}</span>
                </div>
                @if($subject->students_count)
                <div class="flex items-center justify-between text-sm">
                    <span class="text-gray-600">Enrolled Students:</span>
                    <span class="font-medium text-gray-900">{{ $subject->students_count }}</span>
                </div>
                @endif
            </div>
