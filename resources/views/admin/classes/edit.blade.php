...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Edit Class: {{ $class->name }}</h2>
            <p class="mt-1 text-sm text-gray-600">Update class information and settings</p>
        </div>
        <x-modern.button
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <x-modern.input
                        ...
                        name="academic_year"
                        type="text"
                        :value="old('academic_year', $class->academic_year ?? '2024-2025')"
                    ...
                    <x-modern.select
                        label="Class Teacher"
                        name="teacher_id"
                        :value="old('teacher_id', $class->teacher_id ?? null)"
                        hint="Assign a responsible teacher"
                        icon='<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>'
                    >
                        <option value="">Select Teacher</option>
                        @foreach($teachers ?? [] as $teacher)
                            <option value="{{ $teacher->id }}" {{ old('teacher_id', $class->teacher_id ?? null) == $teacher->id ? 'selected' : '' }}>
                    ...
                    <x-modern.select
                        label="Status"
                        name="status"
                        :value="old('status', $class->status ?? 'active')"
                    ...
                    >
                        ...
                        <option value="inactive" {{ old('status', $class->status ?? 'active') == 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </x-modern.select>
                </div>
            </div>
